...
const FeedItem = ({ index }) => {
  ...
  useEffect(() => {
    axios
      .get(`https://group-18.herokuapp.com/reaction/${id}`, config)
      .then(() => setLiked(true))
    ...
    axios
      ...
      .then((response) => {
        ...
        axios
          .get(`https://group-18.herokuapp.com/favorite/${id}`)
          .then((res) => setFav(res.data.length > 0))
          .catch((err) => console.error(err));
      })
      .catch((err) => {
        console.error(err);
    ...
    <Flex my="5" justify="center" w="100%">
      ...
      >
        <Flex w="100%" align="center" my="5">
          ...
          <Text m="3">{feedDetails.title}</Text>
        </Flex>
